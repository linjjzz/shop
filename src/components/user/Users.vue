...
import { getSwitchState } from '../../network/switch'
...
export default {
  ...
  methods: {
    ...
    userStateChanged(userInfo) {
      getSwitchState(userInfo).then((res) => {
        if (res.meta.status !== 200) {
    ...
    saveRoleInfo() {
      ...
      //发起请求保存
      updateRole(this.userInfo.id, this.selectedRoleId).then((res) => {
        console.log(res)
        //判断如果删除失败，就做提示
        if (res.meta.status !== 200) {
